...
namespace SalesAdventure.Entities
{
    public class Cyclop : Creature
    {
        ...
        public override void Attack(Creature target)
        {
            ...
            Math.Round(target.Hp -= cyclopAttack);

            Console.WriteLine($"\n{this.Name}{Game.TextColor} attacks {target.Name}{Game.TextColor} for {Game.HpColor}{cyclopAttack}{Game.TextColor} damage. {target.Name}{Game.TextColor} have now {Game.HpColor}{target.Hp}{Game.TextColor} HP left.");
        }
        ...
        public override void ThrowRock(Creature target)
        {
            ...
            if (isCriticalHit)
            {
                Math.Round(cyclopRock *= 3);
               Console.WriteLine($"\n{this.Name}{Game.TextColor} lands a critical hit with a rock on {target.Name}{Game.TextColor}!");
            }
            Math.Round(target.Hp -= cyclopRock);

            Console.WriteLine($"\n{this.Name}{Game.TextColor} throws a rock on {target.Name}{Game.TextColor} for {Game.HpColor}{cyclopRock}{Game.TextColor} damage. {target.Name}{Game.TextColor} stumbles and have now {Game.HpColor}{target.Hp}{Game.TextColor} HP left.");
        }
        ...
        private void CyclopEncounter(DrawMap drawMap, string[,] map, Player player1, Creature target, Item pie, Item apple)
        {
        ...
        public void CyclopEncount(DrawMap drawMap, string[,] map, Player player1, Creature target, Item pie, Item apple)
        {
            CyclopEncounter(drawMap, map, player1, target, pie, apple);
        }
    }
}
